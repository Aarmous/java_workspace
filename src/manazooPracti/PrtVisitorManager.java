...
import manazooPracti.PrtAnimalEnum.Species;

public class PrtVisitorManager {

	Scanner in = new Scanner(System.in);
	Map<String, PrtReservation> reservations = new HashMap<>();
	...
	public PrtReservation reservation() {
		String id; // 예약번호
		String date;
		int adultCount;
		int childCount;
		...
		while (true) {
			id = PrtIdGeneratorUtil.generateId(); // 예약번호 생성 
			date = inputDate(); // 방문날짜 입력 받기
			adultCount = inputPeopleNumber("대인"); // 대인 인원수 입력 받기
			childCount = inputPeopleNumber("소인"); // 소인 인원수 입력 받기

			// 입력 정보 확정 받고, 결제 여부 결정
			if (confirmInformation(date, adultCount, childCount,
					totalPrice)) {
				break; // while문 깨고 payment() 진행 
			}
		}
		return payment(id, name, phone, date, adultCount, childCount,
				totalPrice);
		...
		while (true) {
			...
			try {
				LocalDate localDate = LocalDate.parse(inDate,
						formatter);
				return localDate.format(formatter); // String 타입의 날짜를 반환 
	...
	public int inputPeopleNumber(String person) { // 대인 or 소인 입력 받기 
		while (true) {
			System.out.printf("%s 인원수 : \n", person);
			String inputPersonCount = in.nextLine();
			if (!StringIsInt(inputPersonCount)) { // int로 바꿀 수 없는 값 예외처리 
				System.out.println("숫자로 정확히 입력해 주세요.");
			} else {
				int intPersonCount = Integer
						.parseInt(inputPersonCount); // int로 바꿀 수 있으면, int로 변환 
				if (intPersonCount >= 0) {
					return intPersonCount; // 입력 받은 인원수를 int 타입으로 반환  
				} else {
	...
	// 입력받은 String 값이 int 로 변환 가능한지 확인하는 메소드 
	...
	public boolean confirmInformation(String date, int adultCount,
			int childCount, int totalPrice) {
	...
	public PrtReservation payment(String id, String name,
			String phone, String date, int adultCount, int childCount,
			int totalPrice) {
		while (true) {
			...
			if (answer.equals("1")) {
				...
				PrtReservation reservation = new PrtReservation(id,
						name, phone, date, adultCount, childCount,
						totalPrice);
		...
		while (true) {
			...
			if (reservations.isEmpty()) {
				...
				if (reservations.containsKey(findId)) {
					...
					// 검색된 예약 내용 보여주기 
					PrtReservation reservation = reservations
							.get(findId);
					System.out.println(reservation);
					return;
				} else {
